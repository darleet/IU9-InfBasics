```
% Лабораторная работа № 5. Интерпретатор конкатенативного языка 
программирования
% 15 января 2023 г.
% Павлов Иван, ИУ9-12Б
```
...
Реализовать интерпретатор стекового языка программирования. Интерпретатор 
должен вызываться как процедура `(interpret program stack)` которая 
принимает программу на исходном языке `program` и начальное состояние стека 
данных `stack` и возвращает его состояние после вычисления программы. 
    ...
    (if (>= index (vector-length program))
        ...
        (let ((lexem (vector-ref program index)))
          (cond ((number? lexem)
                ...
                ((equal? lexem 'and)
                 (main (+ 1 index) 
                       (cons (stack-and (car stack) (cadr stack)) 
                             (cddr stack)) 
                       dict 
                       stack_return))
                ((equal? lexem 'or)
                 (main (+ 1 index) 
                       (cons (stack-or (car stack) (cadr stack)) 
                             (cddr stack)) 
                       dict 
                       stack_return))
                ((equal? lexem 'drop)
                ...
                ((equal? lexem 'if)
                 (if (not (equal? (car stack) 0))
                     (main (+ 1 index) 
                           ...
                           dict 
                           stack_return)
                     (main (find-endif index) 
                           (cdr stack) 
                           dict 
                           stack_return)))
                ((equal? lexem 'endif) 
                 (main (+ 1 index) stack dict stack_return))
                ((equal? lexem 'end) 
                 (main (car stack_return) 
                       stack 
                       dict 
                       (cdr stack_return)))
                ((equal? lexem 'exit) 
                ...
                ((not (equal? (assq lexem dict) #f))
                 (main (cadr (assq lexem dict))
                       stack 
                       dict 
                       (cons (+ 1 index) stack_return)))
                (else 'error)))))
  (main 0 stack '() '()))
```
...
- [x] Реализован собственный интерпретатор стекового ЯП
- [x] В интерпретаторе реализована обработка основных встроенных слов для 
арифметических операций, операций сравнения, логических операций, 
операций со стеком и управляющих конструкций